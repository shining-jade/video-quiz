...
function parseArgs(argv) {
  const result = {
    ...
    targetMode: 'production', gateId: '', windowId: '', controlId: ''
  };
...
async function main(argv = process.argv.slice(2), dependencies = productionDependencies()) {
  ...
  const identity = captureEvidenceIdentity(
    ...
    { tool: 'set-counter-migration-cli', schemaVersion: 2 },
    dependencies.now
  );
  ...
  try {
    ...
    const report = authorEvidenceReport(await dependencies.runCounterBackfill({
