...
> 2026-08-21 공개 퀴즈 자료실의 코드·Rules·자동 검증은 준비됐지만 production migration/deploy/push, 실제 계정, 브라우저 인수는 미실행 운영 gate입니다. [`docs/PUBLIC-QUIZ-LIBRARY.md`](./docs/PUBLIC-QUIZ-LIBRARY.md)의 private-default/no-backfill 계약과 production dry-run → Rules 먼저 → static app → privacy smoke 순서를 지키십시오. 휴지통·purge·교사 중지·탈퇴 대기는 모든 visible publication을 먼저 withdrawn으로 만들며, `remainingVisibleCount === 0`이 아니거나 allowance/auth-generation/route가 바뀌면 상태 변경을 중단합니다. public image orphan audit가 0이 아니면 private parent를 삭제하지 않습니다. 실패 시 이미 withdrawn인 항목을 재공개하지 말고 앱과 호환 Rules만 문서 절차대로 롤백합니다.
