...
> 2026-08-22 공개 퀴즈 자료실의 코드·Rules·자동 검증은 준비됐지만 production audit/migration/deploy/push, 실제 계정, 브라우저 인수는 미실행 운영 gate입니다. [`docs/PUBLIC-QUIZ-LIBRARY.md`](./docs/PUBLIC-QUIZ-LIBRARY.md)의 private-default/no-backfill 계약과 새 durable `audit:public-library` 보고서 → index build 완료 → Rules 먼저 → static app → privacy smoke 순서를 지키십시오. 보고서가 `complete: true`, `safeToDeployPublicLibrary: true`, `findings: []`가 아니면 배포하지 않습니다. public child는 exact `revision + schemaVersion == 1` query만 허용하며 marker가 없는 legacy child를 backfill하지 않습니다. auditor는 gate/direct get까지 포함한 전체 read budget으로 모든 `quiz_sets`와 lifecycle lock을 검사합니다. 교사 중지·탈퇴는 owner lock과 `publication_lifecycle_gates/current`로 공개와 모든 게시/복사 mutation을 먼저 fail-closed하고 최종 allowance transaction에서 둘을 exact consume합니다. stale/malformed gate는 blind delete하지 말고 exact operation 재시도 또는 Admin 조사 뒤 paired 해제합니다. building 중 휴지통 이동은 숨김 `cancelled` tombstone이며 복원 후 새 token으로만 재게시합니다. 실패 시 이미 withdrawn인 항목을 재공개하지 말고 앱과 호환 Rules만 문서 절차대로 롤백합니다.
