...
> 2026-08-22 이메일 인증·교사 access/session·lifecycle·set counter·collaborator share·공개 자료실을 합친 production audit/migration/deploy/provider/browser 인수는 모두 미실행 운영 gate입니다. 전체 순서의 유일한 authoritative source는 [`docs/RELEASE-RUNBOOK.md`](./docs/RELEASE-RUNBOOK.md)입니다. R1 externally enforced exact write-quiescence → R2 lifecycle → R3 share → R4 set counter lock/apply → R5 access lock/apply → R6 session lock/recount/gate → R7 public privacy/author-value audit → R8 index build → R10 strict Rules → R11 static app → R12 같은-generation verify → R13 exact unlock → R14 Email/Password provider → R15 controlled smoke 순서를 축약하지 마십시오. `safeToDeployStrictRules: true`와 `safeToDeployPublicLibrary: true`가 없는 보고서, unsafe/partial 보고서, 다른 UID의 canonical email mirror, unsafe author label/parity, active/stale gate, index building, generation drift가 하나라도 있으면 중단합니다. 롤백도 quiescence를 유지해 직전 호환 Rules를 먼저, 직전 static app을 다음으로 복원하며 데이터·Auth 사용자·allowance·withdrawn publication을 삭제하지 않습니다. 실제 deploy/push/Console/provider/계정/메일/브라우저 작업은 이 구현에서 수행하지 않았습니다.
