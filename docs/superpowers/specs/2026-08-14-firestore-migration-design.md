...
Firestore에는 RTDB의 `.info/serverTimeOffset`이 없으므로 전용 시각 보정 문서를 사용한다. 앱 시작 후 인증이 끝나면 기기마다 충돌하지 않는 고유한 `clock/{sampleId}` 문서에 서버 타임스탬프를 기록하고 완료된 값을 되읽은 뒤 문서를 삭제한다. 되읽은 서버 밀리초와 요청 왕복의 로컬 시작·종료 시각 중간값 차이로 오프셋을 계산해 메모리에 캐시한다.
...
모든 경로는 익명 인증을 포함한 Firebase 인증 사용자를 요구한다. `quiz_sets`는 읽기·생성·수정만 허용하고 삭제는 금지한다. 이미지, 코드, 세션과 모든 세션 하위 컬렉션, config 문서 및 임시 clock 문서는 인증 사용자에게 필요한 읽기·쓰기를 허용한다.
