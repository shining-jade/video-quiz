...
- `published_quiz_sets/{publicationId}`는 비식별 공개 부모다. 허용 필드는 `publicationId`, `sourceSetId`, `status`, `moderationStatus`, `revision`, `title`, `description`, `authorDisplayName`, `revealMode`, `limitSec`, `revealDelaySec`, `autoPause`, `videoCount`, `questionCount`, `imageCount`, `publishedAt`, `updatedAt`뿐이다. `building` 동안에만 `buildToken`, `buildVideoCount`, `buildQuestionCount`, `buildImageCount`, `buildMutation`이 추가된다.
...
소유자는 활성 원본의 정확한 `contentRevision`과 활성 `teacher_allowances`를 서버에서 다시 읽은 뒤 `building` projection을 만들고, 모든 평면 child를 결합한 뒤에만 `published`로 전환한다. 중간 build는 목록과 다른 교사의 get/copy에서 보이지 않는다. 철회는 같은 revision의 `published`를 `withdrawn`으로 바꾸며 child가 남아 있어도 새 조회와 복사는 막힌다.
...
- 교사 중지와 탈퇴 대기는 `withdrawOwnedPublicationsForLifecycle`이 정확한 `ownerUid`로 private source를 50개 이하씩 server-read paging한다. 각 항목에서 source/public revision을 transaction으로 재확인하고, 매 page 및 최종 상태 변경 직전에 시작 allowance의 UID·canonical email·role·revision·status를 다시 확인한다.
...
1. 백업과 기존 migration gate를 확인하고 공개 문서·orphan·PII에 대한 **production dry-run** 보고서를 만든다. 자동 apply는 하지 않는다.
2. 테스트된 `firestore.rules`의 해시와 직전 Rules 릴리스를 기록한 뒤 **Rules를 먼저 배포**한다. Emulator 결과 또는 dry-run 감사가 불완전하면 중단한다.
3. 새 Rules가 적용된 것을 재확인한 뒤에만 **정적 앱을 배포**한다. 앱을 먼저 배포하면 lifecycle paired write가 거부되거나 이전 앱이 공개 상태를 남길 수 있다.
4. 아래 actor matrix로 **privacy smoke**를 완료하고 콘솔에서 앱/Firebase origin 오류가 0인지 확인한다.
...
privacy smoke 또는 수명주기 점검이 실패하면 신규 게시·계정 중지·탈퇴 처리를 중단한다. 데이터나 기존 독립 사본을 되돌리지 않는다. 먼저 정적 앱을 직전 검증 버전으로 롤백하고, 그 앱과 호환되는 직전 Rules 릴리스를 복원한다. 이미 `withdrawn`인 publication은 다시 공개하지 않는다. moderated 항목은 관리자 감사 없이 복구하지 않는다. 실패 batch의 allowance가 active인지, 공개 visible count가 0인지 다시 감사한 후 원인을 수정해 전체 순서를 처음부터 반복한다.
